...
const pokémonNamesWithIdGreaterThan99 = pokémon.filter(p => p.id > 99).map(p => p.name);
console.log(pokémonNamesWithIdGreaterThan99);

const poisonTypePokémonNames = pokémon.filter(p => p.types.length === 1 && p.types[0] === "poison").map(p => p.name);
console.log(poisonTypePokémonNames);

const firstTypeOfFlyingPokémon = pokémon.filter(p => p.types.length === 2 && p.types[1] === "flying").map(p => p.types[0]);
console.log(firstTypeOfFlyingPokémon);

const normalTypePokémonCount = pokémon.filter(p => p.types.includes("normal")).length;
